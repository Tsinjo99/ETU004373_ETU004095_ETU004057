...
class AchatController
{
    ...
    public function store(): void
    {
        ...
        // AUTOMATIQUEMENT créer une distribution
        // Car le produit acheté par BNGRC est distribué à la ville
        $this->distributionModel->create($besoin_id, $don_id, $quantite);

        $this->app->set('flash_message', 'Achat effectué et distribution créée ! Montant: ' . number_format($montant_total, 2) . ' Ar (dont ' . $frais_pourcent . '% de frais). ' . $quantite . ' unités distribuées.');
        $this->app->redirect('/achats');
    }
}
