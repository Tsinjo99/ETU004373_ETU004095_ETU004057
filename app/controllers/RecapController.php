...
class RecapController
{
    ...
    private function calculateRecap(): array
    {
        ...
        // Note: distribue_montant inclut TOUT (dons purs + achats qui créent des distributions)
        foreach ($parVille as &$ville) {
            $ville['satisfait_montant'] = $ville['distribue_montant']; // achats créent des distributions, donc distribue inclut tout
        ...
        // Totaux globaux
        // distributions_total inclut TOUT (dons purs + achats qui créent des distributions)
        // achats_total = argent dépensé (info financière, pas ajouté au satisfait car déjà comptée via distributions)
        $satisfaitTotal = $distribTotal['montant'];
